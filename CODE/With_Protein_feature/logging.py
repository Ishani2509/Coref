		...
		if(line.startswith(" Scored Candidates for")):
			...
			for elem in candidates:
				...
				try:
					score = elem.split("=")[1]
					try:
						isproteinname = elem.split("=")[0].split("|")[1].split("_")[-2]
					except IndexError:
						isproteinname = 0

					if(isproteinname == 'Protein'):
						isprotein = 1
					else:
						isprotein = 0
					...
					score = 'null'
					isthreshold = 0
				list_of_cand_spans.append(names+":"+score+":"+str(isthreshold)+":"+str(istopscore)+":"+mention_span+":"+type_dict[mention_type].split(";")[0]+':'+str(isprotein))
...
with open('Final_Log.txt') as f:
	...
	for line in content.split("\n"):
		...
		if(" Passed Filter 3" in line):
			...
			if(target_cand.startswith('WORD')):
				filter_line[filename][mention_type+":"+mention_span].append(target_cand)
			else:
				filter_line[filename][mention_type+":"+mention_span].append(line.split("_")[-1])
				...
				for elem2 in list(lines[file2].keys()):
					if(elem1==elem2):
						...
						if(len(filter_line[file1][elem1])==0):
							...
							#print(filt_cand)
							if(filt_cand.startswith('WORD')):
								print('WORD')
								print(filt_cand.split("_")[-1][:-1])
							else:
								print('MULTI_WORD')
								print(filt_cand[:-17])
								#print(filt_cand.split("_")[-1][:-1])
							for elem in lines[file1][elem1]['spans']:
								print(elem)
								if(filt_cand.split("_")[-1][:-1] in elem.split(":")[0]):
									print('True')
									write_file.write(elem + ":" + '1'+ ','+ file1 +'\n')
								elif(filt_cand[:-17] in elem.split(":")[0]):
									print('True')
									write_file.write(elem + ":" + '1'+ ','+ file1 +'\n')
								else:
									write_file.write(elem + ":" + '0'+ ','+file1 +'\n')
